...
const items = computed(() => mainStore.account_list);
    ...
    <div class="flex flex-col md:flex-row md:flex-wrap gap-4 p-4">
      ...
      <div class="flex gap-0  justify-start">
        <FormField>
          ...
          <BaseButton type="submit" color="info" class="w-fit text-sm h-9" label="Search" />
        </FormField>
      </div>
    </div>
    <div class=" overflow-x-auto border border-gray-300 bg-white rounded-md m-4 px-4 py-2">
      ...
      <table class="min-w-full">
        <thead class="bg-slate-100 border border-t-0 border-l-0 border-r-0 border-gray-300">
          <tr>
            <th data-label="Account No">Account No</th>
            <th data-label="Name">Name</th>
            <th data-label="Initial Balance">Initial Balance</th>
            <th data-label="Default">Default</th>
        ...
        <tbody>
          <tr v-for="itemslist in itemsPaginated" :key="itemslist.transaction_number">
            ...
            <td data-label="Account No">
              {{ itemslist.account_no }}
            </td>
            <td data-label="Name">
              {{ itemslist.name }}
            </td>
            <td data-label="Initial Balance">
              ₱{{ itemslist.initial_balance.toFixed(2) }}
            </td>
            <td data-label="Default">

              <label class="inline-flex items-center cursor-pointer">
                <input type="checkbox" value="" class="sr-only peer" :checked="itemslist.default">
                <div
