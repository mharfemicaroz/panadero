...
const items = computed(() => mainStore.franchises_po)
...
<template>
  ...
  <div>
    <div class="flex rounded-md overflow-hidden w-fit p-2">
      ...
      <a href="#franchises_po" class="px-6 py-2 font-semibold rounded-md bg-blue-100 text-blue-700">
        Franschisee PO
      </a>
      <a href="#franchises_request" class="px-6 py-2 font-semibold  ">
       Request
      </a>
    </div>
      ...
      <tbody>
        <tr v-for="itemslist in itemsPaginated" :key="itemslist.id">
          ...
          <td data-label="Purchase Order">
            {{ itemslist.purchase_order }}
          </td>
          <td data-label="Date" class="lg:w-32">
            {{ itemslist.date }}
          </td>
          <td data-label="Status" class="lg:w-32">
            <span class="border border-green-400 text-green-600 bg-green-100 px-3 py-1 rounded-md text-sm"> {{
              itemslist.status }}</span>
          </td>
          <td data-label="Excpected on" class="lg:w-32">
            {{ itemslist.expected_on }}
          </td>
          <td class="">
