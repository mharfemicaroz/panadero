...
const items = computed(() => mainStore.bookmarked_templates);
...
<template>
  ...
  <div>
    ...
    <div class="">
      <h5 class="text-md font-bold py-2 px-5 ">Create your template </h5>
      <div class="px-5">

        <div class="text-center bg-red-800 text-white border border-gray-300 rounded p-5 my-2">
          <br>
          <PillTag class="border-0" :huge="true" :icon="mdiPlus" opacity="opacity-50" />
          <br>
          <span class="font-bold text-2xl">Start from scratch</span>
          <br>
          <span class=" text-xm">Get started with a blank template</span>
          <br><br><br>
        </div>
      ...
      <table class="min-w-full border bg-white">
        ...
        <tbody>
          <tr v-for="itemslist in itemsPaginated" :key="itemslist.transaction_number">
            ...
            <td data-label="Name">
              <PillTag class="border-0" :huge="false" :icon="mdiBookmark" opacity="opacity-100" />
              {{ itemslist.name }}
            </td>
            <td data-label="Date">
              {{ itemslist.date }}
            ...
            <td class="before:hidden lg:w-1 whitespace-nowrap">
              <BaseButtons type="justify-start lg:justify-end" no-wrap>
                <BaseButton color="" class="border border-gray-300 bg-white" small label="Start Inspection" />
    ...
    <div class="px-5">
      ...
      <table class="min-w-full border bg-white">
        <thead class="bg-slate-100 border border-t-0 border-l-0 border-r-0 border-gray-300">
          <tr>
            <th data-label="Name">Name</th>
            <th data-label="Last published">Last published</th>
            <th data-label="Access">Access</th>
            <th></th>
          </tr>
        </thead>

        <tbody>
          <tr v-for="itemslist in itemsPaginated2" :key="itemslist.transaction_number">
            <TableCheckboxCell v-if="checkable" @checked="checked($event, itemslist)" />
            <td data-label="Name">
